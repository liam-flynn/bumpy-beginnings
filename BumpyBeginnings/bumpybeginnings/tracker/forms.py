...
class DevelopmentMilestoneForm(forms.ModelForm):
    class Meta:
        ...
        fields = ['stage', 'week', 'start_age_months', 'end_age_months', 'description', 'image']
        widgets = {
    ...
    def __init__(self, *args, **kwargs):
        ...
        used_weeks = DevelopmentMilestone.objects.filter(stage='prenatal').values_list('week', flat=True)

        # create a list of available weeks for prenatal stage
        available_weeks = [(i, f"Week {i}") for i in range(1, 41) if i not in used_weeks]
        ...
        if self.instance and self.instance.week and (self.instance.week, f"Week {self.instance.week}") not in available_weeks:
            available_weeks.append((self.instance.week, f"Week {self.instance.week}"))
...
def clean(self):
    ...
    if stage == 'prenatal':
        if not week or not (1 <= int(week) <= 40):
            self.add_error('week', "Week must be between 1 and 40 for prenatal milestones.")
        ...
        if week not in [None, '']:
            self.add_error('week', "Week should not be set for postnatal milestones.")
        if start_age_months is None or end_age_months is None:
            self.add_error('start_age_months', "Start and end age months must be set for postnatal milestones.")
            self.add_error('end_age_months', "Start and end age months must be set for postnatal milestones.")
        elif start_age_months >= end_age_months:
            self.add_error('start_age_months', "Start age must be less than end age.")
