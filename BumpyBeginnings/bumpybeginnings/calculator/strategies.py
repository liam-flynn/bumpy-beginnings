...
decimal_strategy = st.floats(min_value=0.01, max_value=1000, allow_nan=False, allow_infinity=False)

effective_date_value = now().date().isoformat()

criterion_strategy = st.text(alphabet=string.printable, min_size=1, max_size=100).filter(lambda s: s.strip() != "")
...
reduction_rate_strategy = st.floats(min_value=0.01, max_value=100, allow_nan=False, allow_infinity=False)
